    ...
    <div style={{ background: 'var(--bg-page)', minHeight: '100vh', color: 'var(--text-primary)', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <section style={{ padding: 'clamp(24px,5vw,60px) clamp(16px,4vw,60px)', textAlign: 'center' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p style={{ fontSize: 20, color: 'var(--text-secondary)', maxWidth: 560, margin: '0 auto 60px', lineHeight: 1.6 }}>From craving to doorstep in under 40 minutes. Here&apos;s how we make it royal.</p>
        </motion.div>
          ...
          {steps.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7, fontSize: 16 }}>{s.text}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} style={{ marginTop: 60 }}>
          <Link href="/discover" style={{ background: `linear-gradient(135deg,${PURPLE},${PINK})`, color: 'var(--text-primary)', padding: '18px 48px', borderRadius: 14, fontSize: 18, fontWeight: 800, textDecoration: 'none' }}>
            Order now
          </Link>
        </motion.div>
      </section>
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
          {faqs.map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
              style={{ padding: '24px 28px', background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)' }}>
              <h3 style={{ fontSize: 17, fontWeight: 800, marginBottom: 8 }}>{f.q}</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: 15, lineHeight: 1.6, margin: 0 }}>{f.a}</p>
            </motion.div>
          ))}
        </div>
