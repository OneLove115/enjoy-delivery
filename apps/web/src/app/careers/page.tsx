...
export default function CareersPage() {
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 48 }}>
        {[{ icon: '🌍', title: 'Remote-First', text: 'Work from anywhere in the EU' }, { icon: '🍕', title: 'Free Meals', text: 'EnJoy credit every month' }, { icon: '📈', title: 'Equity', text: 'Stock options for every role' }].map((p, i) => (
          <div key={i} style={{ textAlign: 'center', padding: '24px 16px', background: 'rgba(90,49,244,0.04)', borderRadius: 16 }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>{p.text}</p>
          </div>
        ))}
      </div>
      ...
      {jobs.map((j, i) => (
        <div key={i} style={{ padding: '24px 28px', background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border)', marginBottom: 16, cursor: 'pointer' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 14, marginBottom: 8 }}>{j.desc}</p>
      ...
      <div style={{ textAlign: 'center', marginTop: 40 }}>
        <Link href="/contact" style={{ display: 'inline-block', padding: '14px 36px', background: 'linear-gradient(135deg,#5A31F4,#FF0080)', borderRadius: 40, color: 'var(--text-primary)', fontWeight: 800, fontSize: 16 }}>Apply Now</Link>
      </div>
    </div>
      <Footer />
    </div>
