  ...
  return (
    <div style={{ background: '#0A0A0F', minHeight: '100vh', color: 'white', fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '100px 20px 80px', textAlign: 'center' }}>
        ...
        <Link href="/account" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.45)', fontSize: 14, fontWeight: 700, marginBottom: 40, textDecoration: 'none' }}>
          ← Terug
        </Link>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div style={{
            ...
            fontSize: 13, fontWeight: 900, color: 'white', marginBottom: 20, letterSpacing: '0.5px',
          }}>
            Coming soon
          </div>
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 16, lineHeight: 1.7, maxWidth: 420, margin: '0 auto 48px' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
          style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 48, textAlign: 'left' }}>
          {[
            ...
            <div key={i} style={{ padding: '18px 16px', borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: 24, marginBottom: 10 }}>{f.icon}</div>
              <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 6 }}>{f.title}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>{f.text}</div>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
          style={{ padding: '32px', borderRadius: 20, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          {submitted ? (
            <div>
              <div style={{ fontSize: 40, marginBottom: 12 }}>✅</div>
              <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 8 }}>Je staat op de lijst!</div>
              <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)' }}>We sturen je een mail zodra cadeaukaarten beschikbaar zijn.</div>
            </div>
          ) : (
            <>
              <div style={{ fontSize: 16, fontWeight: 900, marginBottom: 8 }}>Wil je als eerste weten wanneer het live gaat?</div>
              <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', marginBottom: 20 }}>Laat je e-mailadres achter en we laten het je weten.</div>
              <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 10 }}>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
                  placeholder="jouw@email.nl"
                  style={{ flex: 1, padding: '13px 18px', borderRadius: 12, background: '#1a1a2e', border: '1px solid rgba(255,255,255,0.1)', color: 'white', fontSize: 14, outline: 'none', fontFamily: 'inherit' }} />
                <button type="submit"
                  style={{ padding: '13px 22px', borderRadius: 12, background: `linear-gradient(135deg,${PURPLE},${PINK})`, border: 'none', color: 'white', fontWeight: 800, fontSize: 14, cursor: 'pointer', flexShrink: 0 }}>
                  Meld aan
                </button>
              </form>
            </>
