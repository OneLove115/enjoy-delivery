        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button onClick={() => setAdding(!adding)}
              style={{ background: `linear-gradient(135deg,${PURPLE},${PINK})`, color: 'white', border: 'none', borderRadius: 12, padding: '10px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
              + Add address
            </button>
          </div>
          ...
          {adding && (
            <form onSubmit={handleAdd} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', padding: '28px', marginBottom: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <button type="submit"
                style={{ background: `linear-gradient(135deg,${PURPLE},${PINK})`, color: 'white', border: 'none', borderRadius: 12, padding: '13px 0', fontSize: 15, fontWeight: 700, cursor: 'pointer', marginTop: 4 }}>
                Save address
              </button>
            </form>
          )}
              ...
              {addresses.map((a, i) => (
                <motion.div key={a.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div>
                    ...
                    <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, margin: 0 }}>{a.street}, {a.postcode} {a.city}</p>
                  </div>
                </motion.div>
              ))}
            </div>
