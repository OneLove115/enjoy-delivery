...
function getResponse(msg: string, lang: Lang): Message {
  ...
  if (m.includes("pizza") || m.includes("italiaan") || m.includes("italian") || m.includes("italiano")) return { role: "assistant", text: R.pizza[lang], restaurants: RESTAURANTS.pizza };
  if (m.includes("burger") || m.includes("hamburger")) return { role: "assistant", text: R.burger[lang], restaurants: RESTAURANTS.burger };
  if (m.includes("sushi") || m.includes("japans") || m.includes("japanese") || m.includes("japanisch")) return { role: "assistant", text: R.sushi[lang], restaurants: RESTAURANTS.sushi };
  if (m.includes("snel") || m.includes("haast") || m.includes("quick") || m.includes("fast") || m.includes("schnell") || m.includes("سريع")) return { role: "assistant", text: R.fast[lang], restaurants: RESTAURANTS.fast };
  if (m.includes("vegetar") || m.includes("vegan") || m.includes("groen") || m.includes("plant") || m.includes("نباتي")) return { role: "assistant", text: R.veg[lang], restaurants: RESTAURANTS.veg };
  if (m.includes("verras") || m.includes("weet niet") || m.includes("surprise") || m.includes("random") || m.includes("überrasch") || m.includes("مفاجأة")) return { role: "assistant", text: R.surprise[lang], restaurants: RESTAURANTS.surprise };
  if (m.includes("kip") || m.includes("chicken") || m.includes("hähnchen") || m.includes("دجاج")) return { role: "assistant", text: R.chicken[lang], restaurants: RESTAURANTS.chicken };
  if (m.includes("aziatisch") || m.includes("chinees") || m.includes("wok") || m.includes("asian") || m.includes("asiatisch") || m.includes("آسيوي")) return { role: "assistant", text: R.asian[lang], restaurants: RESTAURANTS.asian };
  if (m.includes("goedkoop") || m.includes("budget") || m.includes("gratis bezorg") || m.includes("cheap") || m.includes("günstig") || m.includes("رخيص")) return { role: "assistant", text: R.cheap[lang], restaurants: RESTAURANTS.cheap };
  if (m.includes("drank") || m.includes("drink") || m.includes("bier") || m.includes("wijn") || m.includes("alcohol") || m.includes("beer") || m.includes("wine") || m.includes("مشروب")) return { role: "assistant", text: R.drinks[lang], chips: ["🍷 Wijn", "🍺 Bier", "🥂 Prosecco", "🥤 Frisdrank"] };
  if (m.includes("aanbieding") || m.includes("offer") || m.includes("deal") || m.includes("korting") || m.includes("angebot") || m.includes("عرض")) return { role: "assistant", text: R.offer[lang], restaurants: RESTAURANTS.cheap, chips: ["30% korting 🏷️", "Gratis bezorging ✓", "2+1 gratis 🎁"] };
...
/* ─── Animated waveform bars (mini) ─── */
function WaveformBars({ active }: { active: boolean }) {
  const [heights, setHeights] = useState([4, 4, 4, 4, 4, 4, 4]);
  const reduced = useReducedMotion();
  useEffect(() => {
    if (!active || reduced) { setHeights([4, 4, 4, 4, 4, 4, 4]); return; }
    const iv = setInterval(() => setHeights([...Array(7)].map(() => Math.round(4 + Math.random() * 20))), 130);
    return () => clearInterval(iv);
  }, [active, reduced]);
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 2.5, height: 28, padding: "0 4px" }}>
      {heights.map((h, i) => (
        <div key={i} style={{ width: 3, borderRadius: 3, background: active ? `linear-gradient(to top, ${PURPLE}, ${PINK})` : "rgba(255,255,255,0.25)", height: `${h}px`, transition: "height 0.1s ease, background 0.3s" }} />
      ))}
    </div>
  );
}

/* ─── Full-width voice waveform (canvas) ─── */
function VoiceWaveform({ active, height = 90 }: { active: boolean; height?: number }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const target = (active && !reduced)
        ? Array.from({ length: N }, (_, i) => { const dist = Math.abs(i - N / 2) / (N / 2); return (0.1 + Math.random() * 0.9) * (1 - dist * 0.6); })
        : Array(N).fill(0.05);
      barsRef.current = barsRef.current.map((v, i) => v + (target[i] - v) * 0.18);
      const barW = W / N, cx = W / 2;
      ...
      barsRef.current.forEach((v, i) => {
        const x = i * barW + barW * 0.2, bh = Math.max(4, v * H * 0.85), y = (H - bh) / 2, r = barW * 0.3;
        ctx.fillStyle = grad;
        ctx.beginPath();
  ...
  return <canvas ref={canvasRef} width={400} height={height} style={{ width: "100%", height, display: "block" }} />;
}
...
function speakBrowser(text: string, onStart: () => void, onEnd: () => void) {
  ...
  utt.lang = "nl-NL"; utt.rate = 1.05; utt.pitch = 1.05;
  const voices = synth.getVoices();
  const female = voices.find((v) => v.lang.startsWith("nl") && /female|woman|vrouw/i.test(v.name)) ?? voices.find((v) => v.lang.startsWith("nl")) ?? voices.find((v) => v.lang.startsWith("en"));
...
/* ─── ElevenLabs TTS ─── */
async function speakElevenLabs(text: string, audioRef: React.MutableRefObject<HTMLAudioElement | null>, onStart: () => void, onEnd: () => void) {
  const clean = text.replace(/[\u{1F000}-\u{1FFFF}]/gu, "").replace(/[·•\n]/g, " ").trim().slice(0, 450);
  try {
...
/* ─── Header avatar (small, inside panel) — shares layoutId with launcher ─── */
function PanelJoyaFace() {
  return (
    <motion.svg
      width={38}
      height={38}
    ...
    >
      <circle cx="20" cy="20" r="20" fill="url(#joya-header-grad)" />
      <defs>
        <radialGradient id="joya-header-grad" cx="40%" cy="35%" r="65%">
          <stop offset="0%" stopColor="#7B52FF" />
          <stop offset="100%" stopColor={PURPLE} />
        </radialGradient>
      </defs>
      <ellipse cx="14" cy="17" rx="2.8" ry="2.8" fill="white" />
      <ellipse cx="26" cy="17" rx="2.8" ry="2.8" fill="white" />
...
export function JoyaChatWidget({ triggerOpen = 0 }: { triggerOpen?: number }) {
  ...
  const [chipsSent, setChipsSent] = useState(false); // hides quick-start chips after first send
  ...
  const closePanel = useCallback(() => {
    ...
    setTimeout(() => {
      const fab = document.querySelector<HTMLElement>('[aria-label="Open Joya AI chat"]');
      fab?.focus();
    }, 350);
  }, []);
  ...
  const onSpeakEnd = useCallback(() => {
    setSpeaking(false);
    if (voiceModeRef.current) setTimeout(() => startListening(), 600);
  }, [startListening]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setMessages((current) => {
        ...
        if (welcome && ttsOn) speakElevenLabs(welcome.text, audioRef, () => setSpeaking(true), onSpeakEnd);
        return current;
      });
    }, 600);
  ...
  useEffect(() => {
    ...
    setMessages([{ role: "assistant", text: `${greeting}! Ik ben Joya — jouw persoonlijke eet-concierge. ${tc.suggestion[lang]}`, chips: tc.chips }]);
    const hasOpened = localStorage.getItem("joyaOpened");
    if (!hasOpened && window.innerWidth <= 768) {
      const t = setTimeout(() => { openPanel(); localStorage.setItem("joyaOpened", "1"); }, 1800);
      return () => clearTimeout(t);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const sendToAI = useCallback(async (history: Message[], detected: Lang) => {
    try {
      const res = await fetch("/api/joya", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: history }) });
      if (res.ok) {
        const data = await res.json() as { text: string };
        if (data.text) {
          const lastUserMsg = history.filter((m) => m.role === "user").pop()?.text ?? "";
          const fallback = getResponse(lastUserMsg, detected);
          const resp: Message = { role: "assistant", text: data.text, chips: fallback.chips, restaurants: fallback.restaurants };
          setMessages((p) => [...p, resp]);
          setLoading(false);
          processingRef.current = false;
          if (!open) setHasUnread(true);
          if (ttsOn) speakElevenLabs(resp.text, audioRef, () => setSpeaking(true), onSpeakEnd);
          return;
        }
      }
    } catch { /* fall through */ }
  ...
  /* Quick-start chips (shown only when thread is effectively empty / first message only) */
  const showInitialChips = messages.length <= 1 && !chipsSent;

  /* ── Voice Mode Panel ─────────────────────────────────────────── */
  const voicePanel = (
    <div style={{ position: "fixed", bottom: 58, left: 0, right: 0, zIndex: 9995, background: "linear-gradient(180deg, #05050F 0%, #0E0520 70%, #060610 100%)", borderTop: "1px solid rgba(90,49,244,0.3)", borderRadius: "20px 20px 0 0", display: "flex", flexDirection: "column", alignItems: "center", padding: "14px 0 10px", fontFamily: "Outfit, sans-serif" }}>
      <div style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "center", position: "relative", marginBottom: 3 }}>
        ...
        <button onClick={exitVoiceMode} aria-label="Sluit spraak modus" style={{ position: "absolute", right: 16, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "50%", width: 26, height: 26, cursor: "pointer", color: "rgba(255,255,255,0.5)", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        <button onClick={() => { exitVoiceMode(); openPanel(); }} aria-label="Ga naar tekst chat" style={{ position: "absolute", left: 16, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "rgba(255,255,255,0.4)", fontSize: 11, fontWeight: 700, padding: "4px 10px", cursor: "pointer", fontFamily: "inherit" }}>💬</button>
      </div>
      <div style={{ fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.5)", marginBottom: 6, textAlign: "center", padding: "0 60px", minHeight: 18, lineHeight: "18px" }}>
        {loading ? "Nadenken…" : speaking ? lastJoyaMsg.slice(0, 55) + (lastJoyaMsg.length > 55 ? "…" : "") : listening ? `"${lastUserMsg.slice(0, 45)}${lastUserMsg.length > 45 ? "…" : ""}"` || "Luisteren…" : "Zeg: “Hé Joya, ik heb trek in Sushi”…"}
      </div>
      <div style={{ position: "relative", width: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <VoiceWaveform active={listening || speaking} height={90} />
        <button onClick={toggleVoice} title={listening ? "Stop" : "Spreken"} aria-label={listening ? "Stop luisteren" : "Start spreken"} style={{ position: "absolute", width: 62, height: 62, borderRadius: "50%", border: "none", cursor: "pointer", background: listening ? `linear-gradient(135deg, ${ORANGE}, ${PINK})` : speaking ? `linear-gradient(135deg, ${PINK}, ${PURPLE})` : `linear-gradient(135deg, ${PURPLE}, ${PINK})`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: listening ? `0 0 0 8px rgba(255,107,0,0.15), 0 0 30px rgba(255,107,0,0.45)` : `0 0 0 8px rgba(90,49,244,0.15), 0 0 30px rgba(90,49,244,0.5)`, transition: "all 0.3s ease", zIndex: 2 }}>
          {speaking ? <svg width="22" height="22" viewBox="0 0 24 24" fill="white"><rect x="6" y="4" width="4" height="16" rx="1" /><rect x="14" y="4" width="4" height="16" rx="1" /></svg> : <MicIcon active={listening} />}
        </button>
      </div>
    </div>
  );
  ...
  const chatPanelContent = (
    <>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 16px", background: `linear-gradient(135deg, ${PURPLE}, #3a1fa0)`, borderBottom: "1px solid rgba(255,255,255,.08)", flexShrink: 0 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span translate="no" style={{ fontSize: 11, fontWeight: 900, color: "rgba(255,255,255,0.55)", letterSpacing: "0.5px", display: "inline-flex", alignItems: "center", gap: 3 }}>
                by <img src="/logo-enjoy.png" alt="EnJoy" style={{ height: 18 }} />
              </span>
            </div>
            <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 11 }}>AI concierge · {lang.toUpperCase()}</div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <select value={lang} onChange={(e) => setLang(e.target.value as Lang)} aria-label="Taal kiezen" style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 8, color: "white", fontSize: 11, padding: "4px 6px", cursor: "pointer", fontWeight: 700 }}>
            <option value="nl">NL</option>
            <option value="en">EN</option>
            <option value="de">DE</option>
            <option value="ar">AR</option>
          </select>
          <motion.button onClick={() => setTtsOn((t) => !t)} title={ttsOn ? "Spraak uit" : "Spraak aan"} aria-label={ttsOn ? "Zet spraak uit" : "Zet spraak aan"} whileHover={reduced ? {} : { scale: 1.06 }} whileTap={reduced ? {} : { scale: 0.94 }} style={{ width: 32, height: 32, borderRadius: 9, background: ttsOn ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.25)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>
            {ttsOn ? "🔊" : "🔇"}
          </motion.button>
          <motion.button onClick={enterVoiceMode} title="Spraak-naar-spraak modus" aria-label="Schakel over naar spraak modus" whileHover={reduced ? {} : { scale: 1.06 }} whileTap={reduced ? {} : { scale: 0.94 }} style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,107,0,0.25)", border: `1px solid rgba(255,107,0,0.4)`, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>
            📞
          </motion.button>
          <motion.button onClick={closePanel} aria-label="Sluit chat" whileHover={reduced ? {} : { scale: 1.06 }} whileTap={reduced ? {} : { scale: 0.94 }} style={{ width: 30, height: 30, borderRadius: 9, background: "rgba(255,255,255,.15)", border: "none", color: "#fff", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>
            ✕
          </motion.button>
        </div>
      </div>
      ...
      <div
        role="log"
        aria-live="polite"
        aria-label="Gesprek met Joya"
        style={{ flex: 1, overflowY: "auto", padding: "16px 14px 8px", display: "flex", flexDirection: "column" }}
        dir={lang === "ar" ? "rtl" : "ltr"}
      >
        {/* Initial quick-start chips (staggered entrance, exit when first message sent) */}
        <AnimatePresence>
          {showInitialChips && messages[0]?.chips && (
            <motion.div
              key="initial-chips"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
        ...
        <AnimatePresence initial={false}>
          {messages.map((m, i) => {
            ...
            const delay = m.role === "assistant" && i > 0 ? 0.04 : 0;
            return (
              <motion.div
                key={i}
              ...
              >
                <div style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
                  {m.role === "assistant" && (
                    <div style={{ flexShrink: 0 }}>
                      <img src="/joya.jpg" alt="Joya" aria-hidden="true" style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", objectPosition: "center top" }} />
                    </div>
                  )}
                  <MessageBubble
                    role={m.role}
                    isLast={isLast && m.role === "assistant"}
                    accentGradient={`linear-gradient(135deg,${PURPLE},${PINK})`}
                    onCopy={() => navigator.clipboard.writeText(m.text).catch(() => {})}
                    onRegenerate={m.role === "assistant" && i > 0 ? () => {
                      const prev = messages[i - 1];
                      if (prev?.role === "user") void handleSend(prev.text);
                    } : undefined}
                  >
                    {m.text}
                  </MessageBubble>
                  ...
                  <div style={{ marginTop: 8, marginLeft: lang === "ar" ? 0 : 36, marginRight: lang === "ar" ? 36 : 0, display: "flex", flexDirection: "column", gap: 7 }}>
                    {m.restaurants.map((r, ri) => (
                      <motion.a
                        ...
                        transition={{ delay: ri * 0.07 + 0.1 }}
                        whileHover={reduced ? {} : { scale: 1.01 }}
                        style={{ display: "flex", alignItems: "center", gap: 10, background: "#141428", borderRadius: 12, padding: "9px 12px", border: "1px solid rgba(90,49,244,0.2)", textDecoration: "none" }}
                      >
                ...
                {/* Follow-up chips (not first message initial chips — those are shown above) */}
                {m.chips && !(i === 0 && showInitialChips) && (
                  <div style={{ marginTop: 9, marginLeft: lang === "ar" ? 0 : 36, display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {m.chips.map((c, ci) => (
                      <SuggestionChip key={ci} label={c} index={ci} onClick={() => void handleSend(c)} accentColor={PURPLE} />
                    ))}
                  </div>
                )}
              </motion.div>
        ...
        <AnimatePresence mode="wait">
          {loading && (
            ...
            >
              <img src="/joya.jpg" alt="Joya" aria-hidden="true" style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", objectPosition: "center top" }} />
              <TypingDots accent={PURPLE} label="Joya is aan het typen" />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* Thinking banner */}
      <AnimatePresence>
        {loading && <ThinkingBanner label="Joya denkt na…" accent={PURPLE} />}
      </AnimatePresence>
      ...
      <ChatInputBar
        ...
        leadingSlot={
          <motion.button
            onClick={toggleVoice}
            title="Spraak invoer"
            aria-label={listening ? "Stop spraak invoer" : "Start spraak invoer"}
            whileHover={reduced ? {} : { scale: 1.06 }}
            whileTap={reduced ? {} : { scale: 0.94 }}
            animate={{ background: listening ? PURPLE : "rgba(255,255,255,0.1)", boxShadow: listening ? `0 0 14px rgba(90,49,244,0.6)` : "none" }}
            style={{ width: 32, height: 32, borderRadius: 9, border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "white" }}
          >
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isMobile && !open && !barHidden && (
          <motion.div
            ...
            style={{ position: "fixed", bottom: 66, left: 0, right: 0, zIndex: 9990, background: "rgba(8,8,16,0.97)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)", borderTop: `1px solid rgba(90,49,244,0.35)`, padding: "12px 20px 24px", display: "flex", alignItems: "center", gap: 14 }}
          >
            <div
              onClick={openPanel}
              role="button"
              aria-label="Open Joya chat"
              tabIndex={0}
              onKeyDown={(e) => e.key === "Enter" && openPanel()}
              style={{ display: "flex", alignItems: "center", gap: 14, flex: 1, minWidth: 0, cursor: "pointer" }}
            >
              <img src="/joya.jpg" alt="Joya" style={{ width: 46, height: 46, borderRadius: "50%", objectFit: "cover", objectPosition: "center top", flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <motion.button
              ...
              aria-label="Start spraak modus"
              whileTap={reduced ? {} : { scale: 0.92 }}
              style={{ width: 50, height: 50, borderRadius: "50%", flexShrink: 0, background: listening ? `linear-gradient(135deg, ${ORANGE}, ${PINK})` : `linear-gradient(135deg, ${PURPLE}, ${PINK})`, border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: `0 0 20px rgba(90,49,244,0.5)` }}
            >
              <MicIcon active={listening} />
            </motion.button>
            <motion.button
              onClick={(e) => { e.stopPropagation(); setBarHidden(true); }}
              aria-label="Verberg Joya bar"
              whileTap={reduced ? {} : { scale: 0.92 }}
              style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.5)", fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >✕</motion.button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Chat panel — uses PanelShell for open/close spring + drag-to-close on mobile */}
      {(!voiceMode || !isMobile) && (
        <PanelShell open={open} onClose={closePanel} isMobile={isMobile}>
          {chatPanelContent}
        </PanelShell>
      )}

      {/* Desktop FAB launcher — shares layoutId with PanelJoyaFace for FLIP morph */}
      {!isMobile && (
        <JoyaAvatar
          asLauncher={!open}
          size={42}
          onClick={open ? closePanel : openPanel}
          hasUnread={hasUnread && !open}
          accent={PURPLE}
        />
      )}

      {/* Desktop close FAB when open */}
      <AnimatePresence>
        {!isMobile && open && (
          <motion.button
            key="close-fab"
            ...
            onClick={closePanel}
            aria-label="Sluit Joya chat"
            whileHover={reduced ? {} : { scale: 1.06 }}
            whileTap={reduced ? {} : { scale: 0.92 }}
            style={{ position: "fixed", bottom: 20, right: 20, zIndex: 9999, width: 58, height: 58, borderRadius: "50%", background: "rgba(20,20,40,0.95)", border: "1px solid rgba(90,49,244,0.4)", fontSize: 20, color: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            ✕
          </motion.button>
        )}
