...
export default function SignupPage() {
  ...
  return (
    <div style={{ background: 'var(--bg-page)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Outfit, sans-serif', color: 'var(--text-primary)' }}>
      ...
      <style>{`
        .auth-signup-card input:focus { box-shadow: 0 0 0 2px #5A31F4; border-color: #5A31F4 !important; outline: none; }
      ...
      `}</style>
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="auth-signup-card"
        ...
        <Link href="/" style={{ textDecoration: 'none', display: 'block', textAlign: 'center', marginBottom: 32 }}>
          <div style={{ width: 72, height: 72, borderRadius: '50%', overflow: 'hidden', margin: '0 auto' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? <><span className="auth-spinner" />Creating account...</> : 'Create account — it\'s free'}
          </button>
        </form>
